...
namespace DataAnalyzer
{
    ...
    public static int[] ParseStringArray(string[] inputArray)
        ...
        public static int[][] CompareArrays(int[][] outerLoopArray, int[][] innerLoopArray, int outputArraySize, int outputInnerArraySize, int combFilter, EPurpose purpose)
        {
            ...
            for (int i = 0; i < outerLoopArray.Length; i++)
            {
                for (int j = 0; j < innerLoopArray.Length; j++)
                {
                    ...
                    {
                        if (test == combFilter && (outputArray[i] is null) == false)
                        {
                            for (int k = 0; k < innerLoopArray[j].Length; k++)
                            {
                                outputArray[i][k] = innerLoopArray[j][k];
                            }
                            if (purpose == EPurpose.statistics)
                            {
                                outputArray[i][innerLoopArray[j].Length]++;
                                break;
                            }
                        }
                        else
